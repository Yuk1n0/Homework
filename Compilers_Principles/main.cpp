...
void gammarAnalysis(wnode* head)
{
	...
	while (p)
	{
		...
		switch (mark)
		{
		case -1:cout << '\t' << "语法分析,翻译成功~~~" << endl; table << ++loc; table.close(); return;
		...
		case 3:tops = tops - 1; topc = topc - 0; opC[topc] = 'S';
			...
			break;
		case 4:tops = tops - 3; topc = topc - 2; opC[topc] = 'C'; cout << '\t' << '\t' << " 归约  " << grammar[4].gen << endl;
			...
			break;
		case 5:tops = tops - 3; topc = topc - 2; opC[topc] = 'T'; cout << '\t' << '\t' << " 归约  " << grammar[5].gen << endl;
